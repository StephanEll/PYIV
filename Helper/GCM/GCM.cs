...
public class GCM
{
	...
	/// <summary>
	/// Gets all messages and deletes them
	/// </summary>
	public static List<Dictionary<string, object>> GetReceivedMessages ()
	{
		if (Application.platform == RuntimePlatform.Android) {
			using (AndroidJavaClass cls = new AndroidJavaClass (CLASS_NAME)) {
				List<string> messages = cls.CallStatic<List<string>> ("getReceivedMessages");
				var notificationList = new List<Dictionary<string, object>> ();
				foreach (string message in messages) {
					var notification = MiniJSON.Json.Deserialize (message) as Dictionary<string, object>;
